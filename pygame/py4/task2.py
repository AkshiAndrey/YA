    ...
    def has_path(self, x1, y1, x2, y2):
        ...
        while queue:
            ...
            if current_cell == (x1, y1):
                ...
                self.trice[index_ + 1] = current_cell
                print(self.text)
                print(self.viz)
                return True
            ...
            for index, (dr, dc) in enumerate(neighbors):
                ...
                if 0 <= new_row < rows and 0 <= new_col < cols and self.board[new_row][new_col] == 2 and (
                    ...
                    self.text.add((new_row, new_col))
                    # if self.trice.get(index):
                    #     self.trice[index_].append((new_row, new_col))
                    # else:
                    #     self.trice[index_] = [(new_row, new_col)]
                    visited.add((new_row, new_col))
            index_ += 1
        return False

    def short_trice(self, set_, cur_pos, mouse_pos):
        # print(self.trice)
        pass
            ...
            if self.board[mouse_pos[0]][mouse_pos[1]] == 0:
                if not self.flag_red_circle:
                    ...
                    if self.has_path(*mouse_pos, *self.flag_red_circle):
                        # self.text.append(self.flag_red_circle)
                        self.short_trice(self.trice, self.flag_red_circle, mouse_pos)
                        # print(self.text)
                        self.board[mouse_pos[0]][mouse_pos[1]] = 2
